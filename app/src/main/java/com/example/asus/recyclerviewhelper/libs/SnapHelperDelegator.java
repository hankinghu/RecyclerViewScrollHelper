...
abstract class SnapHelperDelegator extends BaseSnapHelperDelegator {
  ...
  View findSnapView(RecyclerView.LayoutManager layoutManager) {
    OrientationHelper helper =
        layoutManager.canScrollHorizontally() ? OrientationHelper.createHorizontalHelper(
            layoutManager) : OrientationHelper.createVerticalHelper(layoutManager);
    ...
    previousClosestPosition =
        closestPosition == RecyclerView.NO_POSITION ? previousClosestPosition : closestPosition;
  ...
  int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX,
    ...
    boolean forwardDirection =
        layoutManager.canScrollHorizontally() ? velocityX > 0 : velocityY > 0;
    ...
    for (int i = firstExpectedPosition;
      ...
      if (forwardDirection) {
        ...
        if (previousClosestPosition == layoutManager.getItemCount() - 1
            && previousClosestPosition % snapCount != 0) {
          return (previousClosestPosition - previousClosestPosition % snapCount + snapCount)
              - snapCount * factor;
        }
        return previousClosestPosition - snapCount * factor;
      }
   ...
   * {@link SnapHelperDelegator#getContainerPosition(RecyclerView.LayoutManager, OrientationHelper)}
   * and the {@link SnapHelperDelegator#getChildPosition(View, OrientationHelper)}
   *
   * @return the distance to the gravitated snap position
   */
  abstract int getDistance(RecyclerView.LayoutManager layoutManager, View targetView,
      OrientationHelper helper);
   ...
   */
  abstract int getContainerPosition(RecyclerView.LayoutManager layoutManager,
      OrientationHelper helper);
   ...
   */
  abstract boolean shouldSkipTarget(View targetView, RecyclerView.LayoutManager layoutManager,
